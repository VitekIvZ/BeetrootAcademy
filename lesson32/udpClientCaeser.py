...
def udp_client(server_host=HOST, server_port=PORT):
    ...
    key = input("Enter the encryption key (an integer): ")
    message = input("Enter the message to send: ")
    
    # Send the key and message to the server
    data = f"{key}:{message}"
    ...
    print(f"Received encrypted response from {server_address}: {data.decode()}")
